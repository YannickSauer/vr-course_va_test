...
public class VisualAcuityTest : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if(Input.GetKeyDown(KeyCode.RightArrow)) // right arrow is pressed
            {
                CheckResponse(0);
            }
            if(Input.GetKeyDown(KeyCode.UpArrow)) // key up arrow is pressed
            {
                CheckResponse(1);
            }
            if(Input.GetKeyDown(KeyCode.LeftArrow)) // left arrow is pressed
            {
                CheckResponse(2);
            }
            if(Input.GetKeyDown(KeyCode.DownArrow)) // key down arrow is pressed
            {
                CheckResponse(3);
            }
        }
    ...
    void ChangeStimulus()
    {
        ...
        stimulus.transform.rotation = Quaternion.Euler(0,0, randomDirection * 90);
        Debug.Log("Rotation is: " + randomDirection*90);
